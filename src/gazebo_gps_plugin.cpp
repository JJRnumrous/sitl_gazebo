...
void GpsPlugin::OnUpdate(const common::UpdateInfo&){
  ...
  if ((current_time - last_gps_time_).Double() > gps_update_interval_) {
    ...
    if(( current_time.Double() > 2.0f)&&( current_time.Double() < 6.0f)){
      ignition::math::Vector3d& temp = pos_W_I;
      temp.X() = 10.0;
      temp.Y() = 20.0;      
      auto tmplatlon = reproject(pos_W_I);
      gps_msg.set_latitude_deg(tmplatlon.first * 180.0 / M_PI);
      gps_msg.set_longitude_deg(tmplatlon.second * 180.0 / M_PI);
      gps_msg.set_altitude(temp.Z() + alt_home);
      gps_msg.set_eph(std_xy);
      gps_msg.set_epv(std_z);
      gps_msg.set_velocity(0.0);
      gps_msg.set_velocity_east(0.0);
      gps_msg.set_velocity_north(0.0);
      gps_msg.set_velocity_up(0.0);
    }
