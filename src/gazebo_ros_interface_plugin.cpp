...
void GazeboRosInterfacePlugin::GzConnectGazeboToRosTopicMsgCallback(
  ...
  switch (gz_connect_gazebo_to_ros_topic_msg->msgtype()) {
    ...
    case std_msgs::msgs::ConnectGazeboToRosTopic::POSESTAMPED: {
      ConnectHelper<geometry_msgs::msgs::PoseStamped, geometry_msgs::PoseStamped>(
          &GazeboRosInterfacePlugin::GzPoseStampedMsgCallback, this,
          gazeboNamespace, gazeboTopicName, rosTopicName, gz_node_handle_);
      break;
    }
    default:
...
void GazeboRosInterfacePlugin::GzOdometryMsgCallback(GzOdometryMsgPtr& gz_odometry_msg, ros::Publisher ros_publisher) {
  ...
  ros_odometry_msg_.header.frame_id = "MAV_FRAME_VISION_NED";
  // ros_odometry_msg_.child_frame_id = 'MAV_FRAME_VISION_NED';
...
void GazeboRosInterfacePlugin::GzPoseStampedMsgCallback(GzPoseStampedMsgPtr& gz_pose_msg, ros::Publisher ros_publisher)
{
 // We need to convert from a Gazebo message to a ROS message, and then forward
  // the pose message to ROS.

  // ============================================ //
  // =================== HEADER ================= //
  // ============================================ //
  ros_pose_msg_.header.stamp.sec = gz_pose_msg->time_usec() / 1000000.0;
  ros_pose_msg_.header.stamp.nsec = gz_pose_msg->time_usec() * 1000.0;
  ros_pose_msg_.header.frame_id = "MAV_FRAME_VISION_NED";
  // ros_odometry_msg_.child_frame_id = 'MAV_FRAME_VISION_NED';

  // ============================================ //
  // ===================== POSE ================= //
  // ============================================ //
  ros_pose_msg_.pose.position.x = gz_pose_msg->position().x();
  ros_pose_msg_.pose.position.y = gz_pose_msg->position().y();
  ros_pose_msg_.pose.position.z = gz_pose_msg->position().z();

  ros_pose_msg_.pose.orientation.w = gz_pose_msg->orientation().w();
  ros_pose_msg_.pose.orientation.x = gz_pose_msg->orientation().x();
  ros_pose_msg_.pose.orientation.y = gz_pose_msg->orientation().y();
  ros_pose_msg_.pose.orientation.z = gz_pose_msg->orientation().z();
